...
function TeamCard({ member, index }) {
  ...
  return (
    <div
      ref={ref}
      className="fade-up bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:-translate-y-1 hover:shadow-lg hover:border-green-300 transition-all duration-300 flex items-center gap-4"
    >
      <div
        className={`size-14 rounded-full flex items-center justify-center font-display font-black text-lg text-white shrink-0 shadow-md ${avatarClass}`}
      >
...
function StaffCard({ staff, index }) {
  ...
  return (
    <div
      ref={ref}
      className="fade-up bg-linear-to-br from-green-800 to-green-700 rounded-2xl p-6 text-white shadow-lg hover:-translate-y-1 hover:shadow-xl hover:shadow-green-700/30 transition-all duration-300"
    >
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div ref={titleRef} className="fade-up max-w-2xl mb-16">
          ...
          <h2 className="font-display text-4xl lg:text-5xl font-black text-gray-900 leading-tight mb-4">
            Our{" "}
            <span className="text-green-700">Executive Committee</span>
          </h2>
        ...
        <div ref={staffTitleRef} className="fade-up">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {CORE_STAFF.map((staff, i) => (
              <StaffCard key={staff.name} staff={staff} index={i} />
            ))}
          </div>
        </div>
